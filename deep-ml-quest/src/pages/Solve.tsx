...
const Solve = ({ problemData }: { problemData?: any }) => {
  ...
  useEffect(() => {
    ...
    if (problemData) {
      ...
      console.log("Problem from prop:", problemData);
      return;
    }
    ...
    try {
      ...
      if (storedProblem) {
        ...
        console.log("Problem from sessionStorage:", parsedProblem);
      } else {
  ...
  // Filter sections based on title keywords
  const problemSections = problem.data?.sections?.filter((sec: any) =>
    sec.title.toLowerCase().includes("problem") || 
    sec.title.toLowerCase().includes("example") ||
    sec.title.toLowerCase().includes("learn")
  ) || [];

  const solutionSections = problem.data?.sections?.filter((sec: any) =>
    sec.title.toLowerCase().includes("solution") ||
    sec.title.toLowerCase().includes("implementation") ||
    sec.title.toLowerCase().includes("code explanation")
  ) || [];
  ...
  const handleRun = async () => {
    try {
      ...
      console.log("ENCODER",encodedCode)
        ...
        <div className={`p-4 border-b ${borderColor}`}>
          <div className="flex items-center justify-between mb-2">
            <h1 className="text-lg font-semibold">
              {problem.data?.title || problem.folder || "Problem"}
            </h1>
            <button 
          ...
          <div className="flex items-center gap-3 text-sm">
            ...
            <span className={textSecondary}>
              Acceptance: {problem.data?.acceptanceRate ? `${problem.data.acceptanceRate}%` : "N/A"}
            </span>
          </div>
        </div>
              ...
              {problemSections.length > 0 ? (
                problemSections.map((section: any, idx: number) => (
                  <div key={idx}>
                    ...
                    />
                    {section.links && section.links.length > 0 && !section.links[0].url.startsWith('#') && (
                    ...
                    {section.code_blocks && section.code_blocks.length > 0 && (
                      <div className={`mt-4 ${bgCode} p-4 rounded-lg overflow-auto`}>
                        {section.code_blocks.map((block: any, i: number) => (
                          <pre key={i} className={`text-sm font-mono ${textPrimary}`}>
                ...
                <div className={textSecondary}>
                  <p>{problem.data?.description || "No description available."}</p>
                </div>
              )}
            </div>
          )}
            ...
            <div className="space-y-6">
              <div className={`p-4 ${bgSecondary} rounded-lg border ${borderColor}`}>
                <p className={textSecondary}>
                  Editorial content provides in-depth explanations and approaches to solve this problem efficiently.
                </p>
              </div>
              {problemSections.map((section: any, idx: number) => (
                <div key={idx}>
          ...
          {activeTab === "solutions" && (
            <div className="space-y-6">
              {solutionSections.length > 0 ? (
                solutionSections.map((section: any, idx: number) => (
                  <div key={idx}>
                    <h2 className="text-lg font-semibold mb-3">{section.title}</h2>
                    <div
                      className={`prose ${isDark ? 'prose-invert' : ''} max-w-none mb-4`}
                      dangerouslySetInnerHTML={{ __html: section.content }}
                    />
                    {section.code_blocks && section.code_blocks.length > 0 && (
                ...
                <div className={`p-6 text-center ${bgSecondary} rounded-lg`}>
                  <p className={textSecondary}>No solutions available yet.</p>
                </div>
              )}
            </div>
          )}
      ...
      <div className={`w-1/2 flex flex-col ${bgSecondary}`}>
        ...
        <div className={`flex items-center justify-between p-3 border-b ${borderColor}`}>
          ...
          <div className="flex items-center gap-2">
            <button className={`p-2 rounded ${hoverBg}`} title="Settings">
              <Settings size={18} />
            </button>
            <button className={`p-2 rounded ${hoverBg}`} title="Full Screen">
              <BarChart2 size={18} />
            </button>
          </div>
        </div>
